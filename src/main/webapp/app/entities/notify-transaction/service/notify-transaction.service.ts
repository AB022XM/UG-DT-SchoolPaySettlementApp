...
type RestOf<T extends INotifyTransaction | NewNotifyTransaction> = Omit<T, 'dateCreated' | 'timestamp'> & {
...
export class NotifyTransactionService {
  ...
  protected convertDateFromClient<T extends INotifyTransaction | NewNotifyTransaction | PartialUpdateNotifyTransaction>(
    ...
    return {
      ...notifyTransaction,
      dateCreated: notifyTransaction.dateCreated?.format(DATE_FORMAT) ?? null,
  ...
  protected convertDateFromServer(restNotifyTransaction: RestNotifyTransaction): INotifyTransaction {
    return {
      ...
      dateCreated: restNotifyTransaction.dateCreated ? dayjs(restNotifyTransaction.dateCreated) : undefined,
